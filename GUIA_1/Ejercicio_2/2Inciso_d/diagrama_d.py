...
# Puntos críticos para y' = y^5 - 4y^3 - 5y^2
puntos_criticos = [0, 2.464]
nombres_puntos = ['0', '2.46']

# Intervalos e indicaciones de dirección (flechas)
intervalos = [
    (-1.5, 0, 'abajo'),        # y < 0: y' < 0 (baja)
    (0, 2.464, 'abajo'),       # 0 < y < 2.464: y' < 0 (baja)
    (2.464, 4.0, 'arriba')     # y > 2.464: y' > 0 (sube)
]
...
plt.plot([0, 0], [-1.7, 4.2], color='navy', linewidth=1.5, zorder=1)
...
# Guardar y mostrar
plt.tight_layout()
plt.savefig("diagrama_d.png", dpi=300, bbox_inches='tight')
plt.show()

print("¡Diagrama de fase del inciso d generado con éxito!")