...
# Puntos críticos para dP/dt = P*(P - 1)*(2 - P)
puntos_criticos = [0, 1, 2]
nombres_puntos = ['0', '1', '2']

# Intervalos e indicaciones de dirección (flechas)
intervalos = [
    (-0.8, 0, 'arriba'),    # P < 0: dP/dt > 0 (sube)
    (0, 1, 'abajo'),        # 0 < P < 1: dP/dt < 0 (baja)
    (1, 2, 'arriba'),       # 1 < P < 2: dP/dt > 0 (sube)
    (2, 3.0, 'abajo')       # P > 2: dP/dt < 0 (baja)
]
...
plt.text(0, 4.0, 'Diagrama Fase', fontsize=16, fontweight='bold', ha='center', color='navy')

# 1. Dibujar el eje vertical P (recta de fase)
plt.plot([0, 0], [-1.0, 3.2], color='navy', linewidth=1.5, zorder=1)
...
plt.text(0, 3.35, r'$eje\ P$', fontsize=13, ha='center', va='bottom', color='navy', fontstyle='italic')
...
# Guardar y mostrar
plt.tight_layout()
plt.savefig("diagrama_3a.png", dpi=300, bbox_inches='tight')
plt.show()

print("¡Diagrama de fase del punto 3a generado con éxito!")